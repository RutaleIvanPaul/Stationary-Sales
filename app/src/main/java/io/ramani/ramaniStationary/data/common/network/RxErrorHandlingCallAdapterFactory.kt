...
class RxErrorHandlingCallAdapterFactory private constructor(private val networkErrorHandler: NetworkErrorHandler) :
    CallAdapter.Factory() {
    companion object {
        fun create(networkErrorHandler: NetworkErrorHandler) =
            RxErrorHandlingCallAdapterFactory(networkErrorHandler)
    }
    ...
    override fun get(
        ...
        RxCallAdapterWrapper(
            original.get(
                ...
                annotations!!,
                retrofit!!
            ) as CallAdapter<Any, Any>, networkErrorHandler
        )
    ...
    private class RxCallAdapterWrapper<R>(
        ...
        private fun hookErrorHandler(observable: Observable<*>): Any =
            observable.onErrorResumeNext { throwable: Throwable ->
                Observable.error(
                    networkErrorHandler.handleError(throwable)
                )
            }

        private fun hookErrorHandler(flowable: Flowable<*>): Any =
            flowable.onErrorResumeNext { throwable: Throwable ->
                Flowable.error(

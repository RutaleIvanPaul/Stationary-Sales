package io.ramani.ramaniStationary.domainCore.io
...
import io.ramani.ramaniStationary.domainCore.io.exceptions.FileCopyErrorException
import io.ramani.ramaniStationary.domainCore.log.logDebug
...
class FileHelper(val context: Application) : IFileHelper {
    ...
    fun getFileSizeObservable(file: File): Observable<Long> =
        Observable.fromCallable { getFileSize(file) }
    ...
    private fun createOutputFile(outputFileName: String, extension: String): File {
        val name = outputFileName.let {
            if (extension.isNotBlank() && outputFileName.split("\\.")
                    .last() != extension
            ) it.plus(".$extension")
            else it
        }
    ...
    fun refreshMediaScanning(filePath: String) {

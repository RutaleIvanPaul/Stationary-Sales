package io.ramani.ramaniStationary.domainCore.date
...
fun Long.withUTCDate(): Long =
    DateTime.now().withMillis(this).let {
        DateTime.now(DateTimeZone.UTC)
            .withTime(it.hourOfDay, it.minuteOfHour, it.secondOfMinute, it.millisOfSecond).millis
    }
...
fun Long.mergeWithDate(date: Long): Long =
    DateTime.now().withMillis(date).let { dateTmp ->
        ...
        DateTime.now().withDate(dateTmp.year, dateTmp.monthOfYear, dateTmp.dayOfMonth)
            .withTime(
                timeTmp.hourOfDay,
                timeTmp.minuteOfHour,
                timeTmp.secondOfMinute,
                timeTmp.millisOfSecond
            )
            .millis
    }

...
import io.ramani.ramaniStationary.domainCore.date.getHour
import io.ramani.ramaniStationary.domainCore.date.getMinute
import java.util.*
...
fun Fragment.showConfirmDialog(
    message: String,
    onCanceled: () -> Unit = {},
    confirmationButtonText: Int = R.string.yes,
    onConfirmed: () -> Unit
) {
    ConfirmDialog.newInstance(message, onConfirmed, onCanceled, confirmationButtonText)
...
fun Fragment.showDatePicker(
    ...
    minDate: Long = -1,
    maxDate: Long = -1,
    onDateSelected: (Int, Int, Int, Long) -> Unit
) {
    if (isAdded) {
...
fun Fragment.showDatePicker(
    ...
    minDate: Long = -1,
    maxDate: Long = -1,
    disableSelectedDate: MutableList<Calendar>,
    onDateSelected: (Int, Int, Int, Long) -> Unit
) {
    if (isAdded) {
...
fun Fragment.showTimePicker(
    ...
    onTimeSelected: (Int, Int, Long) -> Unit
) {
    if (isAdded) {
        val calendar = Calendar.getInstance()
        if (date > -1) {
